...
/*	
	...
	It is a collection of nodes that contain data, link for next node and previous node.
	
	The common structure of a doubly linked list node is:
	
	[int data, LinkedList next] : * where data stores the value of the node 
				   	   		 	  * next contains the link to next node in the list
				       		 	  * prev contains the link to previous node in the list
...
List* head = NULL; // Variable to store the head of Linked List. DO NOT CHANGE UNTIL THERE IS CHANGE IN THE HEAD NODE.
List* tail = NULL; // Variable to store the tail of Linked List. DO NOT CHANGE UNTIL THERE IS CHANGE IN THE TAIL NODE.
